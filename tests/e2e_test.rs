...
fn gmail_label_delete(account: &str, id: &str) {
    let _ = omega(account)
        .args(["gmail", "labels", "delete", id])
        .output();
}

fn gmail_draft_delete(account: &str, id: &str) {
    let _ = omega(account)
        .args(["gmail", "drafts", "delete", id])
        .output();
}

fn cal_event_delete(account: &str, id: &str) {
    let _ = omega(account)
        .args(["cal", "delete", "primary", id])
        .output();
}
...
fn task_delete(account: &str, compound_id: &str) {
    if let Some((list_id, task_id)) = compound_id.split_once(':') {
        let _ = omega(account)
            .args(["tasks", "delete", list_id, task_id])
            .output();
    }
}
...
fn e2e_gmail_search_threads() {
    let account = require_account!();
    let val = run_json(omega_json(&account).args(["gmail", "search", "in:inbox", "--max", "2"]));
...
fn e2e_gmail_search_messages() {
    let account = require_account!();
    let val = run_json(
        omega_json(&account).args(["gmail", "messages", "search", "in:inbox", "--max", "2"]),
    );
...
fn e2e_gmail_search_pagination() {
    let account = require_account!();
    // First page
    let page1 = run_json(omega_json(&account).args(["gmail", "search", "in:inbox", "--max", "1"]));
    // If there is a nextPageToken, fetch page 2
    if let Some(token) = extract_id(&page1, "nextPageToken") {
        let page2 = run_json(omega_json(&account).args([
            "gmail", "search", "in:inbox", "--max", "1", "--page", &token,
        ]));
...
fn e2e_gmail_get_message() {
    ...
    let search = run_json(
        omega_json(&account).args(["gmail", "messages", "search", "in:inbox", "--max", "1"]),
    );
    let msg_id = search
        .get("messages")
...
fn e2e_gmail_thread_get() {
    let account = require_account!();
    let search = run_json(omega_json(&account).args(["gmail", "search", "in:inbox", "--max", "1"]));
...
fn e2e_gmail_send_to_self() {
    ...
    let output = omega_json(&account)
        .args([
            ...
            &subject,
            "--body",
            "Automated E2E test message — safe to delete.",
        ])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        eprintln!(
            "  [WARN] gmail send failed (may need send scope): {}",
    ...
    let search = run_json(omega_json(&account).args([
        "gmail",
        "search",
...
fn e2e_gmail_labels_crud() {
    ...
    // Create
    let created = run_json(omega_json(&account).args(["gmail", "labels", "create", &label_name]));
    ...
    assert!(
        list.get("labels").is_some(),
        "labels list should have labels key"
    );
...
fn e2e_gmail_drafts_crud() {
    ...
    // Create
    let Some(created) = run_json_or_skip(omega_json(&account).args([
        "gmail",
        "drafts",
        "create",
        "--to",
...
fn e2e_gmail_settings_vacation_get() {
    let account = require_account!();
    let Some(val) =
        run_json_or_skip(omega_json(&account).args(["gmail", "settings", "vacation", "get"]))
    else {
        return;
    };
...
fn e2e_gmail_settings_filters_list() {
    let account = require_account!();
    if let Some(output) =
        run_or_skip(&mut omega_json(&account).args(["gmail", "settings", "filters", "list"]))
    {
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            !stdout.trim().is_empty(),
            "filters list should produce output"
        );
    }
}
...
fn e2e_gmail_settings_forwarding_list() {
    let account = require_account!();
    if let Some(output) =
        run_or_skip(&mut omega_json(&account).args(["gmail", "settings", "forwarding", "list"]))
    {
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            !stdout.trim().is_empty(),
            "forwarding list should produce output"
        );
    }
}
...
fn e2e_gmail_settings_sendas_list() {
    let account = require_account!();
    let Some(output) =
        run_or_skip(&mut omega_json(&account).args(["gmail", "settings", "sendas", "list"]))
    else {
        return;
    };
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn e2e_gmail_settings_delegates_list() {
    let account = require_account!();
    if let Some(output) =
        run_or_skip(&mut omega_json(&account).args(["gmail", "settings", "delegates", "list"]))
    {
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            !stdout.trim().is_empty(),
            "delegates list should produce output"
        );
    }
}
...
fn e2e_gmail_settings_autoforward_get() {
    let account = require_account!();
    if let Some(output) =
        run_or_skip(&mut omega_json(&account).args(["gmail", "settings", "autoforward", "get"]))
    {
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            !stdout.trim().is_empty(),
            "autoforward get should produce output"
        );
    }
}
...
fn e2e_gmail_search_fail_empty() {
    ...
    let output = omega(&account)
        .args([
            "gmail",
            "search",
            "xyznonexistent_e2e_query_99999",
            "--fail-empty",
        ])
        .output()
...
fn e2e_calendar_events_search() {
    let account = require_account!();
    let val = run_json(omega_json(&account).args(["cal", "events", "-q", "test", "--max", "2"]));
    assert!(
        val.is_object(),
        "calendar events search should return a JSON object"
    );
}
...
fn e2e_calendar_event_crud() {
    ...
    // Create
    let created = run_json(omega_json(&account).args([
        "cal",
        "create",
        "--summary",
        &summary,
    ...
    // Get
    let got = run_json(omega_json(&account).args(["cal", "event", "primary", &event_id]));
    assert_eq!(
    ...
    run_success(&mut omega(&account).args([
        "cal",
        "update",
        "primary",
        &event_id,
...
fn e2e_calendar_freebusy() {
    let account = require_account!();
    let val = run_json(omega_json(&account).args([
        "cal",
        "freebusy",
        "primary",
        "--from",
...
fn e2e_calendar_colors() {
    ...
    let val: serde_json::Value =
        serde_json::from_str(&stdout).expect("colors output should be valid JSON");
...
fn e2e_calendar_time() {
    ...
    assert!(
        !stdout.trim().is_empty(),
        "cal time should produce non-empty output"
    );
}
...
fn e2e_calendar_event_pagination() {
    let account = require_account!();
    let page1 = run_json(omega_json(&account).args(["cal", "events", "--max", "1"]));
    if let Some(token) = extract_id(&page1, "nextPageToken") {
        let page2 =
            run_json(omega_json(&account).args(["cal", "events", "--max", "1", "--page", &token]));
        assert!(page2.is_object(), "page2 should be a JSON object");
    }
}
...
fn e2e_drive_search() {
    let account = require_account!();
    let val = run_json(omega_json(&account).args(["drive", "search", "test", "--max", "2"]));
...
fn e2e_drive_list_pagination() {
    let account = require_account!();
    let page1 = run_json(omega_json(&account).args(["drive", "ls", "--max", "1"]));
    if let Some(token) = extract_id(&page1, "nextPageToken") {
        let page2 =
            run_json(omega_json(&account).args(["drive", "ls", "--max", "1", "--page", &token]));
        assert!(page2.is_object(), "page2 should be a JSON object");
    }
}
...
fn e2e_drive_mkdir_and_delete() {
    ...
    // Create folder
    let created = run_json(omega_json(&account).args(["drive", "mkdir", &folder_name]));
...
fn e2e_drive_rename() {
    ...
    // Rename
    run_success(&mut omega(&account).args(["drive", "rename", &file_id, &new_name]));
...
fn e2e_drive_copy() {
    ...
    let copied =
        run_json(omega_json(&account).args(["drive", "copy", &file_id, "--name", &copy_name]));
...
fn e2e_drive_move() {
    ...
    // Create two folders
    let folder_a = run_json(omega_json(&account).args(["drive", "mkdir", &format!("{prefix}_A")]));
    let folder_a_id = extract_id(&folder_a, "id").unwrap();
    let _guard_a = CleanupGuard::new(&account, folder_a_id.clone(), drive_delete);

    let folder_b = run_json(omega_json(&account).args(["drive", "mkdir", &format!("{prefix}_B")]));
    ...
    let file = run_json(omega_json(&account).args([
        "drive",
        "upload",
        tmp.path().to_str().unwrap(),
        "--name",
    ...
    run_success(&mut omega(&account).args(["drive", "move", &file_id, "--parent", &folder_b_id]));
...
fn e2e_drive_share_unshare() {
    ...
    // Share with anyone
    let shared = run_json(omega_json(&account).args([
        "drive", "share", &file_id, "--to", "anyone", "--role", "reader",
    ]));
    ...
    // List permissions
    let perms = run_json(omega_json(&account).args(["drive", "permissions", &file_id]));
    assert!(
        perms.is_object() || perms.is_array(),
        "permissions should be JSON"
    );
...
fn e2e_drive_comments_crud() {
    ...
    if comment_result.status.success() {
        // List comments
        run_success(&mut omega_json(&account).args(["drive", "comments", &file_id, "list"]));
    } else {
...
fn e2e_drive_get_invalid_id() {
    let account = require_account!();
    run_failure(&mut omega_json(&account).args(["drive", "get", "nonexistent_file_id_e2e_12345"]));
}
...
fn e2e_docs_cat() {
    ...
    let Some(doc_id) = docs_create(&account, &title) else {
        return;
    };
...
fn e2e_docs_find_replace() {
    ...
    // Find-replace
    if run_or_skip(&mut omega(&account).args(["docs", "find-replace", &doc_id, "fox", "dog"]))
        .is_none()
    {
        return;
    }
    ...
    assert!(
        stdout.contains("dog"),
        "find-replace should have changed fox to dog"
    );
}
...
fn e2e_docs_sed() {
    ...
    // Sed
    if run_or_skip(&mut omega(&account).args(["docs", "sed", &doc_id, "s/hello/goodbye/"]))
        .is_none()
    {
        return;
    }
    ...
    assert!(
        ...
        "sed should have changed hello to goodbye"
    );
}
...
fn e2e_docs_export() {
    ...
    let export_file = tempfile::Builder::new()
        .suffix(".txt")
        .tempfile()
        .expect("create export temp");
    let export_path = export_file.path().to_str().unwrap().to_string();
    let Some(_) = run_or_skip(&mut omega(&account).args([
        "docs",
        "export",
        &doc_id,
        "--format",
...
fn e2e_docs_comments_crud() {
    ...
    // List comments
    run_success(&mut omega_json(&account).args(["drive", "comments", &doc_id, "list"]));
}
...
fn e2e_sheets_metadata() {
    ...
    let Some(sheet_id) = sheets_create(&account, &title) else {
        return;
    };
...
fn e2e_sheets_update_and_get() {
    ...
    // Read cells
    let val = run_json(omega_json(&account).args(["sheets", "get", &sheet_id, "Sheet1!A1:B2"]));
...
fn e2e_sheets_clear() {
    ...
    // Clear
    run_success(&mut omega(&account).args(["sheets", "clear", &sheet_id, "Sheet1!A1:A2"]));

    settle();

    // Read to verify empty
    let val = run_json(omega_json(&account).args(["sheets", "get", &sheet_id, "Sheet1!A1:A2"]));
...
fn e2e_sheets_insert_rows() {
    ...
    let val = run_json(omega_json(&account).args(["sheets", "get", &sheet_id, "Sheet1!A1:A4"]));
    let values = val.get("values").and_then(|v| v.as_array());
    assert!(
...
fn e2e_sheets_copy() {
    ...
    let copied = run_json(omega_json(&account).args(["sheets", "copy", &sheet_id, &copy_title]));
...
fn e2e_slides_info() {
    ...
    let Some(pres_id) = slides_create(&account, &title) else {
        return;
    };
...
fn e2e_slides_add_and_delete_slide() {
    ...
    // Add a slide
    let added = run_json(omega_json(&account).args(["slides", "add-slide", &pres_id]));
    ...
    if let Some(sid) = slide_id {
        run_success(&mut omega(&account).args(["slides", "delete-slide", &pres_id, sid]));
    }
}
...
fn e2e_slides_update_notes() {
    ...
    if let Some(sid) = slide_id {
        ...
        let slide = run_json(omega_json(&account).args(["slides", "read-slide", &pres_id, sid]));
...
fn e2e_contacts_list() {
    let account = require_account!();
    let val = run_json(omega_json(&account).args(["contacts", "list", "--max", "2"]));
    assert!(
...
fn e2e_contacts_crud() {
    ...
    let resource_name =
        extract_id(&created, "resourceName").expect("contact should have resourceName");
    let _guard = CleanupGuard::new(&account, resource_name.clone(), contact_delete);

    settle();

    // Get
    let got = run_json(omega_json(&account).args(["contacts", "get", &resource_name]));
    assert!(
...
fn e2e_contacts_list_pagination() {
    let account = require_account!();
    let val = run_json(omega_json(&account).args(["contacts", "list", "--max", "1"]));
...
fn e2e_contacts_other_list() {
    let account = require_account!();
    let output =
        run_success(&mut omega_json(&account).args(["contacts", "other", "list", "--max", "2"]));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
...
fn e2e_contacts_directory_list() {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        if check_skip(&output) || stderr.contains("requestSyncToken") || stderr.contains("400") {
            eprintln!(
                "  [SKIP-API-DISABLED] Directory not available (expected for personal accounts)"
            );
            return;
        }
        panic!("contacts directory list failed unexpectedly: {}", stderr);
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        serde_json::from_str::<serde_json::Value>(&stdout).is_ok(),
...
fn e2e_people_search() {
    let account = require_account!();
    let output =
        run_success(&mut omega_json(&account).args(["people", "search", "test", "--max", "2"]));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        serde_json::from_str::<serde_json::Value>(&stdout).is_ok(),
...
fn e2e_tasks_list_crud() {
    ...
    // Create
    let created = run_json(omega_json(&account).args(["tasks", "lists", "create", &list_name]));
    ...
    let _ = omega(&account).args(["tasks", "clear", &list_id]).output();
}
...
fn e2e_tasks_full_lifecycle() {
    ...
    // Add
    let created =
        run_json(omega_json(&account).args(["tasks", "add", &list_id, "--title", &task_title]));
    ...
    // Get
    let got = run_json(omega_json(&account).args(["tasks", "get", &list_id, &task_id]));
    assert_eq!(
    ...
    // Done
    run_success(&mut omega(&account).args(["tasks", "done", &list_id, &task_id]));

    // Undo
    run_success(&mut omega(&account).args(["tasks", "undo", &list_id, &task_id]));
}
...
fn e2e_tasks_list_tasks() {
    ...
    let output =
        run_success(&mut omega_json(&account).args(["tasks", "list", &list_id, "--max", "3"]));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        serde_json::from_str::<serde_json::Value>(&stdout).is_ok(),
...
fn e2e_tasks_add_with_due() {
    ...
    assert!(
        due.contains("2026-12-31"),
        "due should contain the set date"
    );
}
...
fn e2e_tasks_clear_completed() {
    ...
    // Add 2 tasks
    let t1 = run_json(omega_json(&account).args([
        "tasks",
        "add",
        &list_id,
        "--title",
        &unique_name("TaskClear1"),
    ]));
    let t1_id = extract_id(&t1, "id").unwrap();
    let _guard1 = CleanupGuard::new(&account, format!("{list_id}:{t1_id}"), task_delete);

    let t2 = run_json(omega_json(&account).args([
        "tasks",
        "add",
        &list_id,
        "--title",
        &unique_name("TaskClear2"),
    ]));
    ...
    run_success(&mut omega_json(&account).args(["tasks", "get", &list_id, &t2_id]));
}
...
fn e2e_tasks_pagination() {
    ...
    let val = run_json(omega_json(&account).args(["tasks", "list", &list_id, "--max", "1"]));
    if let Some(token) = extract_id(&val, "nextPageToken") {
        let page2 = run_json(
            omega_json(&account).args(["tasks", "list", &list_id, "--max", "1", "--page", &token]),
        );
        assert!(page2.is_object(), "page2 should be a JSON object");
    }
}
...
fn e2e_forms_create_and_get() {
    ...
    let Some(form_id) = forms_create(&account, &title) else {
        return;
    };
...
fn e2e_forms_responses_list() {
    ...
    run_success(&mut omega_json(&account).args(["forms", "responses", "list", &form_id]));
}
...
fn e2e_output_json_mode() {
    let account = require_account!();
    let output = run_success(&mut omega(&account).args(["drive", "ls", "--max", "1", "--json"]));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        serde_json::from_str::<serde_json::Value>(&stdout).is_ok(),
...
fn e2e_output_plain_mode() {
    let account = require_account!();
    let output = run_success(&mut omega(&account).args(["drive", "ls", "--max", "1", "--plain"]));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn e2e_output_csv_mode() {
    let account = require_account!();
    let output = run_success(&mut omega(&account).args(["drive", "ls", "--max", "1", "--csv"]));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn e2e_select_field() {
    let account = require_account!();
    let output = run_success(&mut omega(&account).args([
        "people",
        "me",
        "--json",
        "--select",
...
fn e2e_error_no_auth() {
    ...
    cmd.args([
        "--account",
        "bogus_nonexistent_account@invalid.test",
        "--no-input",
        "drive",
        "ls",
        "--json",
...
fn e2e_error_invalid_service_id() {
    let account = require_account!();
    run_failure(&mut omega_json(&account).args(["drive", "get", "nonexistent_file_id_e2e_99999"]));
}
